...
/*
    namespace utils {
        typedef create_tuple<8, std::size_t>::type _t_neighbours;
    }
*/
    ...
    class RuleFlatBoard {
        public:
            ...
            static constexpr _t_neighbours neighbours(std::size_t N) {
                return std::make_tuple(
                    // left
                    TGrid::is_left(N) ? ((std::size_t)-1) : (N-1),
                    // right
                    TGrid::is_right(N) ? ((std::size_t)-1) : (N+1),
                    // top
                    TGrid::is_top(N) ? ((std::size_t)-1) : (N-width),
                    // top-left
                    TGrid::is_top(N) ? ((std::size_t)-1) : (TGrid::is_left(N) ? ((std::size_t)-1) : (N-width-1)),
                    // top-right
                    TGrid::is_top(N) ? ((std::size_t)-1) : (TGrid::is_right(N) ? ((std::size_t)-1) : (N-width+1)),
                    // bottom
                    TGrid::is_bottom(N) ? ((std::size_t)-1) : (N+width),
                    // bottom-left
                    TGrid::is_bottom(N) ? ((std::size_t)-1) : (TGrid::is_left(N) ? ((std::size_t)-1) : (N+width-1)),
                    // bottom-right
                    TGrid::is_bottom(N) ? ((std::size_t)-1) : (TGrid::is_right(N) ? ((std::size_t)-1) : (N+width+1))
                );
            }
    };
